...
def search_dnsdumpster(self, target):
    ...
    try:
        ...
        self.errors.append("Error in retreiving CSRF Token for DnsDumpster")
        return
    
    cookies = {'csrf_token': csrf_token,}
    data = [('csrfmiddlewaretoken', csrf_token), ('targetip', target)]

    res = requests.post('https://dnsdumpster.com/', headers=headers, cookies=cookies, data=data)

    try:
        scrapped = res.text
        subdomain_finder = re.compile('\">(.*\.' + target + ')<br>')
        links = subdomain_finder.findall(scrapped)
        for domain in links:
            if domain.strip() not in self.domains and domain.endswith("." + target):
                self.domains.append(domain.strip())
    
    except Exception:
        self.errors.append("Error in DnsDumpster")
